...
public class AccessControlManagerTest extends AbstractGalaxyTest {
    ...
    public void testListener() throws Exception {
        ...
        CountingEventListener counter = new CountingEventListener();
        ...
        assertEquals(++runningCount, counter.count);

        accessControlManager.grant(g, Arrays.asList("ITEM_READ"));
        assertEquals(++runningCount, counter.count);

        accessControlManager.revoke(g, Arrays.asList("ITEM_READ"));
        assertEquals(++runningCount, counter.count);
        
        accessControlManager.grant(g, Arrays.asList("ITEM_READ"), "FOO");
        assertEquals(++runningCount, counter.count);
        
        accessControlManager.revoke(g, Arrays.asList("ITEM_READ"), "FOO");
        assertEquals(++runningCount, counter.count);

        accessControlManager.deleteGroup(g.getId());
        assertEquals(++runningCount, counter.count);
    }
    ...
    public static class CountingEventListener {
        public int count;
        public AccessChangeEvent lastEvent;
