...
public class JcrItem extends AbstractItem {
	...
	public String getId() {
		try {
			...
			// this occurs if this item has been deleted. In which case we want
			// to
	...
	private void setInternalProperty(String name, Object value, boolean log) throws PropertyException, PolicyException, AccessException {
		try {
			...
			if (log) {
				// We need to re-extract the property value because we need to
				// log the external value
				getSaveEvents().add(new PropertyChangedEvent(SecurityUtils.getCurrentUser(), this, name, getProperty(name)));
			}
			update();
		} catch (RepositoryException e) {
			throw new RuntimeException(e);
