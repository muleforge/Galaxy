...
//        assertEquals(0, registry.getArtifacts().size());
    ...
    private Artifact initDocument() throws Exception {
        java.util.Collection<org.mule.galaxy.Workspace> workspaces = registry.getWorkspaces();
        assertEquals(1, workspaces.size());
        org.mule.galaxy.Workspace w = workspaces.iterator().next();
        
        Document hello = DOMUtils.readXml(getWsdl());
        
        return registry.createArtifact(w, hello);
    }

    private InputStream getWsdl() {
        return getResourceAsStream("/org/mule/galaxy/wsdl/hello.wsdl");
    }
