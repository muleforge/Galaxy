...
public class ArtifactListPanel extends AbstractShowable implements ClickListener {
    ...
    public ArtifactListPanel(AbstractBrowsePanel browsePanel, RepositoryModule repository) {
        super();
    ...
    public void onClick(Widget sender) {
        ...
        if (sender == bulkEditLink) {
            ...
            galaxy.getPageManager().createPageInfo("bulk-edit", new BulkEditPanel(getSelectedArtifacts(), repository), 0);
            History.newItem("bulk-edit");

            // edit the entire result set
            setEditable(false);
        } else if (sender == editAll) {
            galaxy.getPageManager().createPageInfo("bulk-edit", new BulkEditPanel(searchResults.getQuery(), searchResults.getTotal(), repository), 0);
            History.newItem("bulk-edit");
