...
public class BulkEditPanel extends AbstractErrorShowingComposite
    ...
    public BulkEditPanel(String query, long resultCount, RepositoryModule repository) {
        this(resultCount, repository);
        this.query = query;
    }
    
    public BulkEditPanel(long resultCount, RepositoryModule repository) {
...
//        wrapperPanel.add(createSecurityPanel());
    ...
    protected void onPropertySelect(ListBox w) {
        ...
        renderer = 
            repositoryModule.getPropertyInterfaceManager().createRenderer(propertyDescriptor.getExtension(), 
                                                                    propertyDescriptor.isMultiValued());
        renderer.initialize(repositoryModule, this, null, true);
    ...
    private void doSaveProperty() {
        ...
        if (entryIds != null) {
            service.setProperty(entryIds, propertyDescriptor.getName(), (Serializable)value, callback);
        } else {
            service.setPropertyForQuery(query, propertyDescriptor.getName(), (Serializable)value, callback);
        }
    }
