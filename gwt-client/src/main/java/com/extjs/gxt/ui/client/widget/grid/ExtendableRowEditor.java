...
import com.extjs.gxt.ui.client.event.ExtendableRowEditorEvent;
 ...
 * <br />
 * Kept in original package for visibility constraints.
 */
public class ExtendableRowEditor<M extends ModelData> extends ContentPanel implements ComponentPlugin {
  ...
  public void startEditing(int rowIndex, boolean doFocus) {
    ...
    ExtendableRowEditorEvent ree = new ExtendableRowEditorEvent(this, rowIndex);
  ...
  protected void afterRender() {
    ...
    if (renderButtons) {
      btns.setWidth((getMinButtonWidth() * btns.getItemCount()) + (frameWidth * 2) + (buttonPad * 2 * btns.getItemCount()));
    }
  }
  ...
  protected void onRender(Element target, int index) {
    ...
    final Button[] extraButtons = createExtraButtons();
    btns.setLayout(new TableLayout(2+extraButtons.length));
    ...
    for (final Button button : extraButtons) {
        btns.add(button);
    }
    ...
    if (!bound && monitorValid) {
      ...
      if (monitorTimer == null) {
        monitorTimer = new Timer() {
          @Override
          public void run() {
            ExtendableRowEditor.this.bindHandler();
          }
        };
      }
